...
public class InitUtils {
  ...
  /**
   * Creates a PiActorConfig based on a BSON object. Used for parsing the response from a call to
   * the server.
   *
   ...
   */
  public static PiActorConfig selfRegister(PiActorConfig config)
      throws RESTException, ParseException, IOException, IndexParseException {
    ...
    if (!qrw.isEmpty()) // if we have an existing registration
    {
      ...
      config.setDocussandraUrl(docussandraConfig.getBaseUrl());// the builder object doesn't set the
                                                               // url
    }
    ...
    if (updateUUID == null) {
      ...
      registerDoc.setUuid(updateUUID);// update; this is only to update the timestamps of
                                      // registration
  ...
  public static void setStates(PiActorConfig config, PiActor actor)
    ...
    for (ActorAbility aa : config.getAbilities()) {
      ...
      if (qrw.isEmpty()) {
        ...
        stateDoc.setObjectAsString("{\"name\": \"" + config.getPiName() + "_" + aa.getName()
            + "\", \"state\":\"" + aa.getState() + "\"}");
  ...
  // stolen (then modified) from stackoverflow:
  // http://stackoverflow.com/questions/9481865/getting-the-ip-address-of-the-current-machine-using-java
  /**
   * Returns an <code>String</code> object encapsulating what is most likely the machine's LAN IP
   * address.
   * <p/>
